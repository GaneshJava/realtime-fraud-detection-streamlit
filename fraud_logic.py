...
def evaluate_rules(payload: Dict, currency: str) -> Tuple[List[Dict], str]:
    ...
    device_last_seen = str(payload.get("device_last_seen", "")).lower()
    ...
    device_new = not device_last_seen or device_last_seen == ""
    location_changed = impossible_distance is not None and impossible_distance > 500
    if device_new and location_changed and amt > inr_to_currency(BASE_THRESHOLDS_INR["medium_amount_threshold"], currency):
    ...
    if beneficiaries_added_24h := int(payload.get("beneficiaries_added_24h", 0)) >= 3 and amt > HIGH_AMOUNT_THRESHOLD:
    ...
    if ip_country and declared_country and ip_country != declared_country and channel not in ("bank", "atm"):
        ...
        add_rule("IP / Declared country mismatch", sev,
                 f"IP {ip_country} != declared {declared_country}")

    if failed_logins >= 5:
        add_rule("Multiple failed login attempts", "HIGH", f"{failed_logins} failed login attempts")

    if new_benef and amt >= inr_to_currency(BASE_THRESHOLDS_INR["medium_amount_threshold"], currency):
    ...
    if suspicious_ip_flag and amt > MEDIUM_AMOUNT_THRESHOLD / 4:
        add_rule("IP flagged by threat intelligence", "HIGH", "IP flagged as suspicious and amount non-trivial")
    ...
    # ================= CHANNEL SPECIFIC =================
    if channel in ("credit card", "online purchase"):
    ...
    if channel == "atm" and amt >= ATM_HIGH_WITHDRAWAL:
        add_rule("Large ATM withdrawal", "HIGH", f"ATM withdrawal {amt:.2f} >= threshold {ATM_HIGH_WITHDRAWAL:.2f}")

    if pos_repeat_count >= 10:
        add_rule("POS repeat transactions", "HIGH", f"{pos_repeat_count} rapid transactions at same POS")

    if channel in ("bank", "netbanking"):
...
def score_transaction_ml(supervised_pipeline, iforest_pipeline, model_payload: Dict) -> Tuple[float, float, str]:
    ...
    try:
        ...
        fraud_prob = 0.0
    try:
        anomaly_score = -float(iforest_pipeline.decision_function(model_df)[0])
    except Exception:
        anomaly_score = 0.0
